...
import com.testautomation.api.clients.ProductsClient;
import com.testautomation.core.BaseApiTest;
...
public class ProductsApiTest extends BaseApiTest {
    ...
    @Test
    void negative_shouldReturnNoProductForUnknownProduct() {
        var products = new ProductsClient(spec);

        // FakeStore returns 200 with no product payload for unknown products instead of 404.
        var resp = products.getProduct(9999);

        assertThat(resp.statusCode())
                .as("FakeStore keeps unknown product lookup successful at HTTP level")
                .isEqualTo(200);
        assertThat(resp.asString())
                .as("Unknown product response body")
                .isIn("", "null");
    }
}
